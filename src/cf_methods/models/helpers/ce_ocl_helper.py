...
# Directly copied from https://github.com/tabearoeber/CE-OCL/blob/main/src/embed_mip.py
...
def optimization_MIP(model,
                     ...
                     enlarge_tr = [False, 0],  ## bool variable for the use of enlarged trust region constraints
    ...
    def constraints_tree(model, outcome, tree_table, lb=None, ub=None, M=1e5, weight_objective=0, SCM=None, features=None):
        ...
        def constraintsTree_1(model, j):
            return sum((model.x[i]+model.e[i]) * coeff.loc[j, i] for i in features) <= intercept.iloc[j] + M*(1-model.l[(outcome,str(l_ids.iloc[j]))])
    ...
    def constraints_tr(model, data, clustering_model, enlarge=False):
        '''
        Add constraints for the trust region using the convex hull of 'data'.
        If enlarge is True, the convex hull is enlarged with an l1norm penalty term in the objective function
        '''
        samples = data.index
        ...
        if enlarge:
            ...
            beta = 100000
            print(f'The trust region is being enlarged with penatly Beta: {beta}.')
            model.OBJ.set_value(expr=model.OBJ.expr + beta*sum(model.eHelp[i] for i in data.columns))

            def constraint_CTR21(model, i):
                return model.eHelp[i] >= model.e[i]

            def constraint_CTR22(model, i):
                return model.eHelp[i] >= - model.e[i]

            model.ConstraintClusteredTrustRegion21 = Constraint(data.columns, rule=constraint_CTR21)
            model.ConstraintClusteredTrustRegion22 = Constraint(data.columns, rule=constraint_CTR22)

            model.add_component('ConstraintClusteredTrustRegion0', Constraint(rule=sum(model.lam[k] for k in samples) == 1))
            model.add_component('ConstraintClusteredTrustRegion1', Constraint(data.columns, rule=constraint_CTR1))
        else:
            def constraint_CTR1(model, i):
                return model.x[i] + model.e[i] == sum(model.lam[k] * data.loc[k, i] for k in samples)

            print('The trust region is not being enlarged.')
            def constraint_CTR2(model, i):
                return model.e[i] == 0
            model.ConstraintClusteredTrustRegion2 = Constraint(data.columns, rule=constraint_CTR2)
    ...
    if tr:
        constraints_tr(model, data, clustering_model, enlarge=enlarge_tr)
